...
<div class="kt-pagebody">
    <form id="topic-form" action="{{ route('edit-topic-form') }}" method="PUT">
        @csrf
        @if(isset($success))
        <h1>Success</h1>
        <script>
                ...
                <div class="row mg-b-25">
                    <div class="col-12">
                        <div class="form-group mg-b-10-force">
                            ...
                            <input value="{{ $topic->id }}" id="id" name="id" required type="hidden">
                            <input value="{{ $topic->label }}" class="form-control " data-placeholder="Title" aria-hidden="true" id="label" name="label" required type="text" name="">
                    ...
                    <div class="col-lg-4">
                        <div class="form-group mg-b-10-force">
                            ...
                            <select class="form-control select2 select2-hidden-accessible" data-placeholder="Select Content Type" aria-hidden="true" id="category" name="category" required>
                                ...
                                <option value="{{ $item->name }}" {{ $topic->category == $item->name ? 'selected' : '' }}>{{ $item->label}}</option>
                                @endforeach
                            </select>
                        </div>
                    </div>
                    ...
                    <div class="col-lg-4">
                        <div class="form-group mg-b-10-force">
                            ...
                            <select class="form-control select2 select2-hidden-accessible" data-placeholder="Select Sub-Category" aria-hidden="true" id="sub_category" name="sub_category" required>
                                <option label="Select Category" disabled>Select Sub-Category</option>
                                @foreach($sub_categories as $item)
                                <option value="{{ $item->name }}" {{ $topic->sub_category == $item->name ? 'selected' : '' }}>{{ $item->label}}</option>
                                @endforeach
                            </select>
                        </div>
                    </div>
